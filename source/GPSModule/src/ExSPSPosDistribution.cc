...
///////////////////////////////////////////////////////////////////////////////
//
// MODULE:        ExSPSPosDistribution.cc
//
// Version:      1.0
// Date:         5/02/04
// Author:       Fan Lei 
...
//
// 2016, Linyan, Modified to include CopyFromSPSP.
//
///////////////////////////////////////////////////////////////////////////////
//
...
ExSPSPosDistribution::ExSPSPosDistribution()
  : posRndm(0)
{
...
void ExSPSPosDistribution::SetPosDisType(G4String PosType)
{
  SourcePosType = PosType;
}

void ExSPSPosDistribution::SetPosDisShape(G4String shapeType)
{
  Shape = shapeType;
}

void ExSPSPosDistribution::SetCentreCoords(G4ThreeVector coordsOfCentre)
{
  CentreCoords = coordsOfCentre;
}

void ExSPSPosDistribution::SetPosRot1(G4ThreeVector posrot1)
{
  // This should be x'
  Rotx = posrot1;
  if(verbosityLevel == 2)
...
void ExSPSPosDistribution::SetPosRot2(G4ThreeVector posrot2)
{
...
void ExSPSPosDistribution::SetHalfX(G4double xhalf)
{
  halfx = xhalf;
}

void ExSPSPosDistribution::SetHalfY(G4double yhalf)
{
  halfy = yhalf;
}

void ExSPSPosDistribution::SetHalfZ(G4double zhalf)
{
  halfz = zhalf;
}

void ExSPSPosDistribution::SetRadius(G4double rds)
{
  Radius = rds;
}

void ExSPSPosDistribution::SetRadius0(G4double rds)
{
  Radius0 = rds;
}

void ExSPSPosDistribution::SetBeamSigmaInR(G4double r)
{
  SX = SY = r;
  SR = r;
}

void ExSPSPosDistribution::SetBeamSigmaInX(G4double r)
{
  SX = r;
}

void ExSPSPosDistribution::SetBeamSigmaInY(G4double r)
{
  SY = r;
}

void ExSPSPosDistribution::SetParAlpha(G4double paralp)
{
  ParAlpha = paralp;
}

void ExSPSPosDistribution::SetParTheta(G4double parthe)
{
  ParTheta = parthe;
}

void ExSPSPosDistribution::SetParPhi(G4double parphi)
{
  ParPhi = parphi;
}

void ExSPSPosDistribution::GenerateRotationMatrices()
{
...
void ExSPSPosDistribution::ConfineSourceToVolume(G4String Vname)
{
  VolName = Vname;
  if(verbosityLevel == 2)
  ...
  else if( theRequiredVolumeName != "NULL")
    {
      G4cout << " **** Error: Volume does not exist **** " << G4endl;
	  G4cout<<theRequiredVolumeName <<G4endl;
...
void ExSPSPosDistribution::GeneratePointSource()
{
...
void ExSPSPosDistribution::GeneratePointsInBeam()
{
  G4double x, y, z;
...
void ExSPSPosDistribution::GeneratePointsInPlane()
{
  G4double x, y, z;
  G4double expression;
  G4ThreeVector RandPos;
...
void ExSPSPosDistribution::GeneratePointsOnSurface()
{
...
void ExSPSPosDistribution::GeneratePointsInVolume()
{
  G4ThreeVector RandPos;
  G4double tempx, tempy, tempz;
  G4double x, y, z;
  ...
  else if(Shape == "SphericalShell")
  {
      G4double RRndm = pow(G4UniformRand()*(pow(Radius, 3)-pow(Radius0, 3))+pow(Radius0, 3),1./3);
      G4double ThetaRndm = acos(2*G4UniformRand()-1);
      G4double PhiRndm = 2*pi*G4UniformRand();
      x = RRndm*sin(ThetaRndm)*cos(PhiRndm);
      y = RRndm*sin(ThetaRndm)*sin(PhiRndm);
      z = RRndm*cos(ThetaRndm);
  }
  else if(Shape == "Ellipsoid")
    {
      G4double temp;
...
G4bool ExSPSPosDistribution::IsSourceConfined()
{
  ...
  if(verbosityLevel >= 1)G4cout<<"volname: "<<theVolName<<G4endl;
...
G4ThreeVector ExSPSPosDistribution::GenerateOne()
{
  //
...
void ExSPSPosDistribution::CopyFromSPSP(ExSPSPosDistribution* pos1){
	SetPosDisType(pos1->GetPosDisType());
	SetPosDisShape(pos1->GetPosDisShape());
	SetCentreCoords(pos1->GetCentreCoords());
	SetHalfX(pos1->GetHalfX());
	SetHalfY(pos1->GetHalfY());
	SetHalfZ(pos1->GetHalfZ());
	SetRadius(pos1->GetRadius());
	SetRadius0(pos1->GetRadius0());
	ConfineSourceToVolume(pos1->GetConfineVolume());
}
