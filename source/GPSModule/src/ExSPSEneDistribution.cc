...
///////////////////////////////////////////////////////////////////////////////
//
// MODULE:        ExSPSEneDistribution.cc
//
// Version:      1.0
// Date:         5/02/04
// Author:       Fan Lei 
...
ExSPSEneDistribution::ExSPSEneDistribution(): Epnflag(false),eneRndm(0),Splinetemp(0)
{
    G4MUTEXINIT(mutex);
	//
...
ExSPSEneDistribution::~ExSPSEneDistribution()
{
    f->Close();
...
void ExSPSEneDistribution::SetEnergyDisType(G4String DisType) {
    G4AutoLock l(&mutex);
...
void ExSPSEneDistribution::SetFileName(G4String filename) {
    //G4AutoLock l(&mutex);
	//RootFileName = filename;
    //threadLocalData.Get().f = new TFile(filename);
    f = new TFile(filename, "READ");
}

void ExSPSEneDistribution::SetHistName(G4String histname) {
    //G4AutoLock l(&mutex);
	HistName = histname;
    h = (TH1*)f->Get(histname);
    ...
    //threadLocalData.Get().h = (TH1*)f->Get(histname);
}

void ExSPSEneDistribution::SetHistUnit(G4String histunit) {
    G4AutoLock l(&mutex);
	HistUnit = histunit;
    threadLocalData.Get().HistUnit = histunit;
}

G4String ExSPSEneDistribution::GetEnergyDisType()
{
    G4AutoLock l(&mutex);
    return EnergyDisType;
}

void ExSPSEneDistribution::SetEmin(G4double emi) {
    G4AutoLock l(&mutex);
	Emin = emi;
    threadLocalData.Get().Emin = Emin;
}

G4double ExSPSEneDistribution::GetEmin()
{
    return threadLocalData.Get().Emin;
}

G4double ExSPSEneDistribution::GetArbEmin()
{
    G4AutoLock l(&mutex);
    return ArbEmin;
}

G4double ExSPSEneDistribution::GetArbEmax()
{
    G4AutoLock l(&mutex);
    return ArbEmax;
}

void ExSPSEneDistribution::SetEmax(G4double ema) {
    G4AutoLock l(&mutex);
	Emax = ema;
    threadLocalData.Get().Emax = Emax;
}

G4double ExSPSEneDistribution::GetEmax()
{
    return threadLocalData.Get().Emax;
}


void ExSPSEneDistribution::SetMonoEnergy(G4double menergy) {
    G4AutoLock l(&mutex);
	MonoEnergy = menergy;
}

void ExSPSEneDistribution::SetBeamSigmaInE(G4double e) {
    G4AutoLock l(&mutex);
	SE = e;
}
void ExSPSEneDistribution::SetAlpha(G4double alp) {
    G4AutoLock l(&mutex);
	alpha = alp;
    threadLocalData.Get().alpha = alpha;
}

void ExSPSEneDistribution::SetBiasAlpha(G4double alp) {
    G4AutoLock l(&mutex);
	biasalpha = alp;
	Biased = true;
}

void ExSPSEneDistribution::SetTemp(G4double tem) {
	G4AutoLock l(&mutex);
    Temp = tem;
}

void ExSPSEneDistribution::SetEzero(G4double eze) {
	G4AutoLock l(&mutex);
    Ezero = eze;
    threadLocalData.Get().Ezero = Ezero;
}

void ExSPSEneDistribution::SetGradient(G4double gr) {
	G4AutoLock l(&mutex);
    grad = gr;
    threadLocalData.Get().grad = grad;
}

void ExSPSEneDistribution::SetInterCept(G4double c) {
	G4AutoLock l(&mutex);
    cept = c;
    threadLocalData.Get().cept = cept;
}

G4String ExSPSEneDistribution::GetIntType()
{
	G4AutoLock l(&mutex);
    return IntType;
}

void ExSPSEneDistribution::SetBiasRndm(G4SPSRandomGenerator* a)
{
    G4AutoLock l(&mutex);
    eneRndm = a;
}

void ExSPSEneDistribution::SetVerbosity(G4int a)
{
    G4AutoLock l(&mutex);
    verbosityLevel = a;
}

G4double ExSPSEneDistribution::GetWeight()
{
    return threadLocalData.Get().weight;
}

G4double ExSPSEneDistribution::GetMonoEnergy()
{
    G4AutoLock l(&mutex);
    return MonoEnergy;
}

G4double ExSPSEneDistribution::GetSE()
{
    G4AutoLock l(&mutex);
    return SE;
}

G4double ExSPSEneDistribution::Getalpha()
{
    return threadLocalData.Get().alpha;
}

G4double ExSPSEneDistribution::GetEzero()
{
    return threadLocalData.Get().Ezero;
}

G4double ExSPSEneDistribution::GetTemp()
{
    G4AutoLock l(&mutex);
    return Temp;
}

G4double ExSPSEneDistribution::Getgrad()
{
    return threadLocalData.Get().grad;
}

G4double ExSPSEneDistribution::Getcept()
{
    return threadLocalData.Get().cept;
}

G4PhysicsOrderedFreeVector ExSPSEneDistribution::GetUserDefinedEnergyHisto()
{
    G4AutoLock l(&mutex);
    return UDefEnergyH;
}

G4PhysicsOrderedFreeVector ExSPSEneDistribution::GetArbEnergyHisto()
{
    G4AutoLock l(&mutex);
    return ArbEnergyH;
}

void ExSPSEneDistribution::UserEnergyHisto(G4ThreeVector input) {
	G4AutoLock l(&mutex);
    G4double ehi, val;
	ehi = input.x();
	val = input.y();
...
void ExSPSEneDistribution::ArbEnergyHisto(G4ThreeVector input)
{
    G4AutoLock l(&mutex);
	G4double ehi, val;
	ehi = input.x();
...
void ExSPSEneDistribution::ArbEnergyHistoFile(G4String filename)
{
    G4AutoLock l(&mutex);
	std::ifstream infile(filename, std::ios::in);
	if (!infile)
    {
		G4Exception("ExSPSEneDistribution::ArbEnergyHistoFile","Event0301",FatalException,"Unable to open the histo ASCII file");
    }
	G4double ehi, val;
...
void ExSPSEneDistribution::EpnEnergyHisto(G4ThreeVector input)
{
    G4AutoLock l(&mutex);
	G4double ehi, val;
	ehi = input.x();
...
void ExSPSEneDistribution::Calculate()
{
    G4AutoLock l(&mutex);
...
//MT: Lock in caller
void ExSPSEneDistribution::BBInitHists()
{
...
//MT: Lock in caller
void ExSPSEneDistribution::CPInitHists()
{
...
//MT: Lock in caller
void ExSPSEneDistribution::CalculateCdgSpectrum()
{
...
//MT: Lock in caller
void ExSPSEneDistribution::CalculateBbodySpectrum()
{
...
//MT: Lock in caller
void ExSPSEneDistribution::CalculateCPowSpectrum()
{
...
void ExSPSEneDistribution::InputEnergySpectra(G4bool value) {
    G4AutoLock l(&mutex);
...
void ExSPSEneDistribution::InputDifferentialSpectra(G4bool value) {
    G4AutoLock l(&mutex);
...
void ExSPSEneDistribution::ArbInterpolate(G4String IType) {
    G4AutoLock l(&mutex);
	if (EnergyDisType != "Arb")
        G4Exception("ExSPSEneDistribution::ArbInterpolate",
...
//MT: Lock in caller
void ExSPSEneDistribution::LinearInterpolation() {
	...
	if (EnergySpec == false)
    {
		...
		if (pdef == NULL)
        {
            G4Exception("ExSPSEneDistribution::LinearInterpolation",
                        "Event0302",FatalException,
...
//MT: Lock in caller
void ExSPSEneDistribution::LogInterpolation()
{
	...
	if (EnergySpec == false)
    {
		...
		if (pdef == NULL)
        {
            G4Exception("ExSPSEneDistribution::LogInterpolation",
                        "Event0302",FatalException,
                        "Error: particle not defined");
        }
		else
...
//MT: Lock in caller
void ExSPSEneDistribution::ExpInterpolation()
{
	...
	if (EnergySpec == false)
    {
		...
		if (pdef == NULL)
        {
            G4Exception("ExSPSEneDistribution::ExpInterpolation",
                        "Event0302",FatalException,
                        "Error: particle not defined");
        }
		else
...
//MT: Lock in caller
void ExSPSEneDistribution::SplineInterpolation()
{
	...
	if (EnergySpec == false) {
        ...
        G4ParticleDefinition* pdef = threadLocalData.Get().particle_definition;
		if (pdef == NULL)
                    G4Exception("ExSPSEneDistribution::SplineInterpolation",
                                "Event0302",FatalException,
			        "Error: particle not defined");
		else {
			// Apply Energy**2 = p**2c**2 + m0**2c**4
	  ...
	  for (count = 0; count < 101; count++) {
	    ...
	    if (prob[count] < 0.) { 
	      ...
	      ED << "Warning: G4DataInterpolation returns value < 0  " << prob[count] <<" "<<ei[count]<< G4endl;
              G4Exception("ExSPSEneDistribution::SplineInterpolation","Event0303",
              FatalException,ED);
	    }
	    area += prob[count]*de;
	  }
...
void ExSPSEneDistribution::GenerateMonoEnergetic() {
	// Method to generate MonoEnergetic particles.
    threadLocalData.Get().particle_energy = MonoEnergy;
}

void ExSPSEneDistribution::GenerateGaussEnergies() {
...
void ExSPSEneDistribution::GenerateLinearEnergies(G4bool bArb = false) {
	G4double rndm;
...
void ExSPSEneDistribution::GeneratePowEnergies(G4bool bArb = false)
{
...
void ExSPSEneDistribution::GenerateCPowEnergies()
{
...
void ExSPSEneDistribution::GenerateBiasPowEnergies()//G4double& ene,G4double& wweight)
{
  // Method to generate particle energies distributed as
...
void ExSPSEneDistribution::GenerateExpEnergies(G4bool bArb = false)
{
...
void ExSPSEneDistribution::GenerateExtHistEnergies(G4bool bArb = false)
{
	// Method to generate particle energies distributed according
	// to a histrogram from an external root file.

    threadLocal_t& params = threadLocalData.Get();
    //G4cout<<"Read a hist"<<G4endl;
    params.particle_energy = h->GetRandom();
    if(HistUnit=="eV")
        params.particle_energy *= eV; 
    else if(HistUnit=="keV")
        params.particle_energy *= keV; 
    else if(HistUnit=="MeV")
        params.particle_energy *= MeV; 
    else if(HistUnit=="GeV")
        params.particle_energy *= GeV; 
    else if(HistUnit=="TeV")
        params.particle_energy *= TeV; 
...
void ExSPSEneDistribution::GenerateBremEnergies()
{
	// Method to generate particle energies distributed according
	...
	if (expmax == 0.)
    {
        G4Exception("ExSPSEneDistribution::GenerateBremEnergies",
                    "Event0302",FatalException,
...
void ExSPSEneDistribution::GenerateBbodyEnergies()
{
...
void ExSPSEneDistribution::GenerateCdgEnergies() {
...
void ExSPSEneDistribution::GenUserHistEnergies()
{
	...
	if (IPDFEnergyExist == false)
    {
        ...
        {
            G4Exception("ExSPSEneDistribution::GenUserHistEnergies",
                        "Event0302",FatalException,
...
void ExSPSEneDistribution::GenArbPointEnergies()
{
	if (verbosityLevel > 0)
    {
    ...
    {
        G4Exception("ExSPSEneDistribution::GenArbPointEnergies","Event0302",
                    FatalException,"Error: IntType unknown type");
    }
}

void ExSPSEneDistribution::GenEpnHistEnergies() {
...
//MT: lock in caller
void ExSPSEneDistribution::ConvertEPNToEnergy()
{
	...
	else
    {
		...
		if (maxcount > 1024)
        {
            G4Exception("ExSPSEneDistribution::ConvertEPNToEnergy()","gps001", JustWarning,
                        "Histogram contains more than 1024 bins!\nThose above 1024 will be ignored");
			maxcount = 1024;
		}
		if (maxcount < 1)
        {
            G4Exception("ExSPSEneDistribution::ConvertEPNToEnergy()","gps001", FatalException,"Histogram contains less than 1 bin!\nRedefine the histogram");
			return;
		}
...
//
void ExSPSEneDistribution::ReSetHist(G4String atype)
{
    G4AutoLock l(&mutex);
	if (atype == "energy")
    {
...
G4double ExSPSEneDistribution::GenerateOne(G4ParticleDefinition* a)
{
    ...
    while ((EnergyDisType == "Arb") ? (params.particle_energy < ArbEmin || params.particle_energy > ArbEmax) : (params.particle_energy < params.Emin|| params.particle_energy > params.Emax))
    {
        ...
        else
        {
			...
			else if (EnergyDisType == "ExtHist")
            {
				GenerateExtHistEnergies(false);
            }
...
G4double ExSPSEneDistribution::GetProbability(G4double ene)
{
    G4double prob = 1.;
    ...
    else if (EnergyDisType == "Arb")
    {
        ...
        if (prob <= 0.)
        {
            //G4cout << " Warning:ExSPSEneDistribution::GetProbability: prob<= 0. "<<prob <<" "<<ene << " " <<deltaY<< G4endl;
            G4cout << " Warning:ExSPSEneDistribution::GetProbability: prob<= 0. "<<prob <<" "<<ene << G4endl;
            prob = 1e-30;
        }
        // already normalised
    }
